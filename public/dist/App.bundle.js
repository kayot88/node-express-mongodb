...
function autocomplite(input, latInput, lngInput) {
  if (!input) return;
  var dropdown = new google.maps.places.Autocomplete(input);

  dropdown.addListener('place_changed', function (e) {
...
function typeAhead(search) {
  if (!search) return;
  var inputSearch = search.querySelector('input[name="search"]');
  ...
  inputSearch.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        searchResult.innerHTML = _dompurify2.default.sanitize(searchResultHTML(res.data));
        // searchResult.firstChild.classList.add(activeClass);
      } else {
        searchResult.innerHTML = _dompurify2.default.sanitize('<div class="search__result">There no data for ' + _this.value + '</div>');
      };
    }).catch(function (err) {
      console.error(err);
    });
...
var _autocomplite = __webpack_require__(10);
...
(0, _autocomplite2.default)((0, _bling.$)('#address'), (0, _bling.$)('#lng'), (0, _bling.$)('#lat'));
